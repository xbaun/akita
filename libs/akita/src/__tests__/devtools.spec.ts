...
describe('DevTools', () => {
  ...
  describe('custom action', () => {
    it(`logAction() should log custom action and payload`, () => {
      store.add([{ id: 1 }]);
      connectMock.send.mockReset();
    ...
    it(`@action() should log custom action with callee arguments as payload`, () => {
      store.add([{ id: 1 }]);
      connectMock.send.mockReset();

      store.updateTitleWithCustomAction('title 1');

      expect(connectMock.send.mock.calls[0][0]['type']).toEqual(expect.stringContaining(buildActionTypeString(store, `updateTitleWithCustomAction`)));
      expect(connectMock.send.mock.calls[0][0]['payload']).toEqual(expect.arrayContaining(['title 1']));
    });

    it(`@action() should log custom action with custom payload`, () => {
      store.add([{ id: 1 }]);
      connectMock.send.mockReset();

      store.updateTitleWithCustomActionPayload('title 1');

      expect(connectMock.send.mock.calls[0][0]['type']).toEqual(expect.stringContaining(buildActionTypeString(store, `updateTitleWithCustomActionPayload`)));
      expect(connectMock.send.mock.calls[0][0]['payload']).toEqual(expect.objectContaining({ title: 'title @action' }));
    });
  });
